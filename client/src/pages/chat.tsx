      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 scroll-smooth" style={{ maxHeight: "calc(100vh - 140px)" }}>
        ...
        {messages.length === 0 && (
          <div className="mb-6 text-center">
            <div className="inline-block bg-chat-bg rounded-lg px-4 py-2 mb-2">
              <p className="text-sm text-text-secondary">👋 Welcome! I'm your AI assistant. How can I help you today?</p>
            </div>
          </div>
        )}
            ...
            {message.sender === "ai" ? (
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  <div className="bg-chat-bg rounded-lg rounded-tl-none px-4 py-3 max-w-xs md:max-w-lg">
                    <p className="text-sm text-text-primary whitespace-pre-wrap">{message.content}</p>
                  </div>
                  <p className="text-xs text-text-secondary mt-1 ml-1">{formatTime(message.timestamp)}</p>
                </div>
              </div>
            ) : (
              <div className="flex items-start space-x-3 justify-end">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-text-secondary mt-1 mr-1 text-right">{formatTime(message.timestamp)}</p>
                </div>
        ...
        {isTyping && (
          <div className="mb-4 animate-in slide-in-from-bottom-2 duration-300">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                <div className="bg-chat-bg rounded-lg rounded-tl-none px-4 py-3 max-w-xs">
        ...
        <form onSubmit={handleSubmit} className="flex items-end space-x-3">
          <div className="flex-1 relative">
            <Textarea
              ...
              className="resize-none border border-gray-300 rounded-lg px-4 py-3 pr-12 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200 text-sm min-h-[44px] max-h-[120px]"
              rows={1}
              maxLength={1000}
